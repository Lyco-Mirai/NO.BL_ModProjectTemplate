...
namespace NuclearOption.NetworkTransforms
{
	public class UnitNetworkTransform : NetworkTransformBase
	{
		...
		public override void Receive(double timestamp, NetworkReader reader)
		{
			...
			if (NetworkFloatHelper.Validate(globalPosition, logErrors: true, "Unit.Position") && NetworkFloatHelper.Validate(value, logErrors: true, "Unit.Rotation"))
			{
			...
			{
				Debug.LogError("Ignoring invalid Unit snapshot from server");
			}
		}
		...
		public override void VisualUpdate(ref VisualUpdateTime visualTime)
		{
			...
			{
				if (!Unit.rb.isKinematic && !Unit.LocalSim && TryGetSnapshot(ref visualTime, out var snapshot))
				{
					ApplySnapshot(snapshot, visualTime.snap);
				}
			}
		}

		private void ApplySnapshot(ViewSnapshot snapshot, bool snap)
		{
