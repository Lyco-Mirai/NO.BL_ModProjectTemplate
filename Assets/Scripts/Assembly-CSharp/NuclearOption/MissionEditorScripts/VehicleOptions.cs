...
{
	public class VehicleOptions : UnitPanelOptions
	{
		[Header("Hold")]
		[SerializeField]
		...
		{
			targets.SetupToggle(holdPosition, holdPositionDifferentValue, (SavedUnit x) => ref ((SavedVehicle)x).holdPosition);
			targets.SetupSlider(skillSlider, skillSliderLabel, (SavedUnit x) => ref ((SavedVehicle)x).skill, (float v) => $"{v:F1}");
		}
